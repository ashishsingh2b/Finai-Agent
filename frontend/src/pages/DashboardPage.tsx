 ...
 * Provides a high-level overview of recent analyses, risk trends, 
 * and institutional portfolio health metrics.
 */
...
export const DashboardPage: React.FC = () => {
    ...
    /**
     * Dashboard Initialization Lifecycle.
     * Fetches current portfolio analyses and initializes global listeners.
     */
    useEffect(() => {
    ...
    const fetchAnalyses = async () => {
        try {
            ...
            console.error('Portfolio retrieval failure:', error);
    ...
    /**
     * Institutional Status Management.
     * Synchronizes manual application status updates with the backend registry.
     */
    const handleStatusUpdate = async (id: number, status: string, currentBehavior: string) => {
        try {
            ...
            // Trigger local hydration for UI synchronicity
            fetchAnalyses();
     ...
     * Records institutional payment behavior for approved credit lines.
     */
    ...
    const getCategoryStyles = (category: string) => {
        const styles: Record<string, string> = {
            'A': 'bg-[#6ECEB2]/20 text-[#11303B] border-[#6ECEB2]/50 ring-[#6ECEB2]/30 font-bold',
            'B': 'bg-blue-50 text-blue-700 border-blue-100 ring-blue-500/20',
            'C': 'bg-amber-50 text-amber-700 border-amber-100 ring-amber-500/20',
            'D': 'bg-orange-50 text-orange-700 border-orange-100 ring-orange-500/20',
            'E': 'bg-red-50 text-red-700 border-red-100 ring-red-500/20',
        };
        return styles[category] || 'bg-gray-50 text-gray-700 border-gray-100 ring-gray-500/20';
    };

    /**
     * Binary Report Extraction.
     * Handles contextual report downloads for specific portfolio entities.
     */
    const handleDownload = async (id: number, type: 'pdf' | 'excel', companyName: string) => {
        try {
            ...
            console.error(`Report generation interruption [${type}]:`, error);
            alert(t('analysis.downloadFailed', { type: type.toUpperCase() }));
        }
    };

    // --- Dynamic Statistical Interpretation ---
    // Calculates temporal trends over 7 and 14-day rolling windows.
    const now = new Date();
    ...
    return (
        <DashboardLayout>
            {/* Executive Welcome & Global Commands */}
                            ...
                            <div>
                                <div className="mb-1 sm:mb-1.5 opacity-60">
                                    <svg className={`w-full h-6 sm:h-8 ${stat.color === 'blue' ? 'text-blue-400' : stat.color === 'emerald' ? 'text-emerald-400' : 'text-red-400'} fill-current`} viewBox="0 0 300 80" preserveAspectRatio="none">
                                        <path d={i === 0 ? "M 0 50 Q 50 40 100 55 T 200 45 T 300 50 L 300 80 L 0 80 Z" : i === 1 ? "M 0 60 Q 75 30 150 50 T 300 40 L 300 80 L 0 80 Z" : "M 0 40 Q 100 60 200 40 T 300 60 L 300 80 L 0 80 Z"} opacity="0.2" />
                                        <path d={i === 0 ? "M 0 50 Q 50 40 100 55 T 200 45 T 300 50" : i === 1 ? "M 0 60 Q 75 30 150 50 T 300 40" : "M 0 40 Q 100 60 200 40 T 300 60"} fill="none" stroke="currentColor" strokeWidth="2.5" />
                                    </svg>
                                </div>
                                <div className="text-[6px] sm:text-[7px] font-black text-gray-400 pt-1 sm:pt-1.5 border-t border-gray-100 uppercase tracking-widest truncate">{stat.footer}</div>
                            </div>
                        </div>
                    </div>
                ))}

                {/* Total Active Portfolio Card (Visual Liquidity Indicator) */}
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-1">
                                ...
                                <div className="flex items-center gap-0.5 sm:gap-1 bg-emerald-50 px-1.5 sm:px-2 py-0.5 rounded-full">
                                    ...
                                    <span className="text-[8px] sm:text-[9px] font-bold text-[#6ECEB2]">+12%</span>
                                </div>
                            </div>
                        ...
                        <div>
                            <div className="mb-1 sm:mb-1.5">
                                <svg className="w-full h-6 sm:h-8" viewBox="0 0 300 80" preserveAspectRatio="none">
                                    <path d="M 0 50 Q 30 45 60 48 T 120 45 T 180 42 T 240 38 T 300 35" fill="none" stroke="#6ECEB2" strokeWidth="3" />
                                    <path d="M 0 50 Q 30 45 60 48 T 120 45 T 180 42 T 240 38 T 300 35 L 300 80 L 0 80 Z" fill="url(#gradient-compact)" opacity="0.2" />
                                    <defs>
                            ...
                            <div className="text-[6px] sm:text-[8px] font-black text-gray-400 pt-1 sm:pt-1.5 border-t border-gray-100 uppercase tracking-widest truncate">{t('dash.systemRecords')}</div>
                        </div>
                    </div>
                </div>
            </div>

            {/* Operational Audit Table: Detailed Portfolio Exploration */}
            <div className="space-y-4">
                ...
                {error && (
                    <div className="bg-red-500/10 border border-red-500/20 text-red-400 px-4 py-3 rounded-xl flex items-center justify-between gap-3 text-xs font-bold animate-in slide-in-from-top-2 duration-300">
                        ...
                        <button onClick={fetchAnalyses} className="bg-red-500/20 hover:bg-red-500/30 px-3 py-1 rounded-lg transition-colors">{t('dash.retry')}</button>
                    </div>
                )}
                ...
                <div className="bg-white rounded-[1.5rem] border border-gray-100 shadow-sm overflow-hidden min-h-[500px]">
                    {loading ? (
                        ...
                        <div className="h-[500px] flex flex-col items-center justify-center gap-4">
                            ...
                            <button onClick={() => navigate('/dashboard/upload')} className="text-[#11303B] font-black text-[9px] uppercase tracking-widest py-2 px-4 bg-[#11303B]/5 rounded-xl hover:bg-[#11303B]/10">{t('dash.start')}</button>
                        </div>
                    ) : (
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {analyses.map((analysis) => (
                                        <tr key={analysis.id} onClick={() => navigate(`/analysis/${analysis.id}`)} className="hover:bg-gray-50/50 transition-colors cursor-pointer group">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center font-black text-[#11303B] text-xs group-hover:bg-white group-hover:shadow-sm transition-all">{analysis.company_name.charAt(0)}</div>
                                            ...
                                            <td className="px-4 py-4" onClick={(e) => e.stopPropagation()}>
                                                <select
                                                    ...
                                                    className={`w-full px-3 py-1.5 rounded-lg text-[9px] font-black border tracking-wider cursor-pointer shadow-sm outline-none transition-all ${analysis.application_status === 'APPROVED' ? 'bg-[#5aac44] text-white border-[#5aac44]' : analysis.application_status === 'REJECTED' ? 'bg-[#ef4444] text-white border-[#ef4444]' : 'bg-[#fbbf24] text-white border-[#fbbf24]'}`}
                                                >
                                            ...
                                            <td className="hidden lg:table-cell px-4 py-4" onClick={(e) => e.stopPropagation()}>
                                                <select
                                                    ...
                                                    className={`w-full px-3 py-1.5 rounded-lg text-[9px] font-black border tracking-wider shadow-sm outline-none transition-all appearance-none cursor-pointer ${analysis.application_status !== 'APPROVED' ? 'bg-gray-50 text-gray-400 border-gray-100 cursor-not-allowed' : 'bg-white text-[#11303B] border-[#11303B]/20 hover:border-[#11303B] hover:shadow-md'}`}
                                                >
